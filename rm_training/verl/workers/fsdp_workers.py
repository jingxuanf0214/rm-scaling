    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            if vllm_mode == "customized":
                ...
                from verl.workers.rollout.vllm_rollout import vLLMAsyncRollout, vLLMRollout
...
class RewardModelWorker(Worker):
    ...
    def _build_model(self, config):
        ...
        if os.path.exists(local_path):
            if os.path.isdir(local_path):
                ...
                model_files = [f for f in contents if 'model' in f.lower() or '.safetensors' in f or '.bin' in f]
            else:
                print(f"[DEBUG] local_path is a file, size: {os.path.getsize(local_path)}")
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            # If target tokenizer has a chat template, use it; otherwise, fall back to raw text
            if hasattr(target_tokenizer, "chat_template") and target_tokenizer.chat_template is not None:
                prompt_text = target_tokenizer.apply_chat_template(chat, add_generation_prompt=False, tokenize=False)
                if self.rank == 0 and i == 0:
                    # for debugging purpose
                    print(f"Switch template (with chat template). chat: {prompt_text}")
            else:
                # Fallback: build a plain text prompt from the message contents
                ...
                prompt_text = " ".join(_extract_text(m) for m in chat if isinstance(m, dict))
                if self.rank == 0 and i == 0:
                    print(f"Switch template fallback (no chat_template). text: {prompt_text[:256]}...")
            ...
            model_inputs = target_tokenizer(prompt_text, return_tensors="pt", add_special_tokens=False)
