...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(
        ...
        exec = inference_engine.llm_engine.model_executor
        if hasattr(exec, "driver_worker"):
            # legacy: vLLM <0.6 where driver_worker is a thing
            driver = exec.driver_worker
            self.model_runner = driver.worker.model_runner
        else:
            # new: SPMDGPUExecutor in v0.6+ — grab the runner directly
            # v0.6.3 exports it either at exec.model_runner or exec.worker.model_runner
            if hasattr(exec, "model_runner"):
                self.model_runner = exec.model_runner
            else:
                self.model_runner = exec.worker.model_runner
