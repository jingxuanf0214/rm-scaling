...
def process_validation_metrics_posneg(
    ...
    """
    Compute simple global averages over the whole validation set for the
    minibatch-level metrics recorded in reward_extra_info:
      - bt_loss
      - avg_pos_rm_score
      - avg_neg_rm_score

    Returns a mapping compatible with the trainer's expectation:
      { "all": { "rm_score": { "bt_loss/mean": ..., "avg_pos_rm_score/mean": ..., "avg_neg_rm_score/mean": ... } } }
    """
    import numpy as np

    bt_list = infos_dict.get("bt_loss", [])
    pos_list = infos_dict.get("avg_pos_rm_score", [])
    neg_list = infos_dict.get("avg_neg_rm_score", [])

    def safe_mean(x):
        return float(np.mean(x)) if len(x) > 0 else float("nan")

    metrics = {
        "bt_loss/mean": safe_mean(bt_list),
        "avg_pos_rm_score/mean": safe_mean(pos_list),
        "avg_neg_rm_score/mean": safe_mean(neg_list),
    }

    return {"all": {"rm_score": metrics}}
