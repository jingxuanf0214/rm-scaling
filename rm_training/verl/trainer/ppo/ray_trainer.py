...
def combine_prompt_target_batch(gen_batch: DataProto, config) -> DataProto:
    ...
    prompt_ids      = gen_batch.batch["input_ids"]         # (Lp,)
    prompt_mask     = gen_batch.batch["attention_mask"]    # (Lp,)
    prompt_pos      = gen_batch.batch["position_ids"]      # (Lp,)
    ...
    eos_mask = torch.isin(resp_ids, torch.tensor(eos_token_id, device=resp_ids.device)).int()
    response_attention_mask = (eos_mask.cumsum(dim=1) - eos_mask).eq(0).to(prompt_mask.dtype)
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.hybrid_engine:
            # Only assert ActorRollout an actor is expected to be enabled
            if hasattr(config, 'trainer') and hasattr(config.trainer, 'enable_actor') and config.trainer.enable_actor:
                assert Role.ActorRollout in role_worker_mapping, f"Role.ActorRollout missing when config.trainer.enable_actor is True. role_worker_mapping.keys()={role_worker_mapping.keys()}"
            elif not (hasattr(config, 'trainer') and hasattr(config.trainer, 'enable_actor')):
                # If enable_actor flag is not present, fall back to original behavior
                assert Role.ActorRollout in role_worker_mapping, f"Role.ActorRollout missing and config.trainer.enable_actor not found. role_worker_mapping.keys()={role_worker_mapping.keys()}"
    ...
    def _validate_config(self):
        ...
        if not config.actor_rollout_ref.actor.use_dynamic_bsz:
            if config.trainer.enable_actor:
    ...
    def _validate_for_rm(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # Store original inputs TODO: add it back later, temporarily disabled it for convenience
            input_ids = test_batch.batch["input_ids"]
            # TODO: Can we keep special tokens except for padding tokens?
            input_texts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in input_ids]
            sample_inputs.extend(input_texts)
            ...
            #batch_keys_to_pop = ["input_ids", "attention_mask", "position_ids"]
            batch_keys_to_pop = ["input_ids", "attention_mask", "position_ids","chosen_input_ids","chosen_attention_mask","chosen_position_ids"]
            non_tensor_batch_keys_to_pop = ["raw_prompt_ids"]
            if "multi_modal_inputs" in test_batch.non_tensor_batch:
                non_tensor_batch_keys_to_pop.extend(["multi_modal_data", "multi_modal_inputs"])
            if "raw_prompt" in test_batch.non_tensor_batch:
            ...
            if not self.async_rollout_mode:
                #test_output_gen_batch_padded = self.actor_rollout_wg.generate_sequences(test_gen_batch_padded)
                test_output_gen_batch = combine_prompt_target_batch(test_gen_batch, self.config)
            else:
                self.async_rollout_manager.wake_up()
                #test_output_gen_batch_padded = self.async_rollout_manager.generate_sequences(test_gen_batch_padded)
                self.async_rollout_manager.sleep()
            repeat_factor = int(self.config.data.val_batch_size * self.config.data.oversample_factor)
            test_batch = test_batch.repeat(repeat_times=repeat_factor, interleave=True)
            # unpad
            #test_output_gen_batch = unpad_dataproto(test_output_gen_batch_padded, pad_size=pad_size)
            print("validation generation end")

            # Store generated outputs TODO: add it back later, temporarily disabled it for convenience
            # output_ids = test_output_gen_batch.batch["responses"]
            # output_texts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in output_ids]
            # sample_outputs.extend(output_texts)
            
            # get similarity scores: this is to identify positive samples
            test_batch = test_batch.union(test_output_gen_batch)
            scores = self.reward_fn.verify(test_batch)
            test_batch = self.filter_and_downsample(scores, test_batch)

            # evaluate using reward_function
            result = self.rm_wg.forward_rm(test_batch)
            # Support both DataProto and dict returns from RM worker
            if isinstance(result, DataProto):
                reward_tensor = result.batch["reward_tensor"]
                extra_info_dict = {k: list(v) for k, v in result.non_tensor_batch.items()} if hasattr(result, "non_tensor_batch") and result.non_tensor_batch is not None else {}
            else:
                reward_tensor = result["reward_tensor"]
                extra_info_dict = result.get("reward_extra_info", {})
            rm_scores = reward_tensor.sum(-1).cpu().tolist()
            sample_scores.extend(rm_scores)

            # Populate extra infos from RM result (e.g., rm_score, acc)
            if extra_info_dict:
                for key, lst in extra_info_dict.items():
                    reward_extra_infos_dict[key].extend(lst)
        ...
        # Ensure extra info lengths align with collected sample scores
        for key_info, lst in reward_extra_infos_dict.items():
            assert len(lst) == 0 or len(lst) == len(sample_scores), f"{key_info}: {len(lst)=}, {len(sample_scores)=}"

        data_sources = np.concatenate(data_source_lst, axis=0)

        from verl.trainer.ppo.metric_utils import process_validation_metrics_posneg
        data_src2var2metric2val = process_validation_metrics_posneg(data_sources, sample_inputs, reward_extra_infos_dict, acc_threshold=0.9)
        metric_dict = {}
        for data_source, var2metric2val in data_src2var2metric2val.items():
            core_var = "rm_score"
            for var_name, metric2val in var2metric2val.items():
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            # Only create ActorRollout worker if actor is enabled
            if hasattr(self.config, 'trainer') and hasattr(self.config.trainer, 'enable_actor') and self.config.trainer.enable_actor:
            ...
            elif not (hasattr(self.config, 'trainer') and hasattr(self.config.trainer, 'enable_actor')):
                # If enable_actor flag is not present, fall back to original behavior
                resource_pool = self.resource_pool_manager.get_resource_pool(Role.ActorRollout)
                actor_rollout_cls = RayClassWithInitArgs(
                    cls=self.role_worker_mapping[Role.ActorRollout],
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # Debug print to check keys in gen_batch non_tensor_batch
                print("gen_batch non_tensor_batch keys:", list(gen_batch.non_tensor_batch.keys()))
                ...
                with _timer("step", timing_raw):
                    ...
                    # After generation and union
                    if raw_prompt_backup is not None:
                        # Repeat the raw_prompt_backup to match the repeated batch size
                        repeat_times = self.config.actor_rollout_ref.rollout.n
                        repeated_raw_prompt = np.repeat(raw_prompt_backup, repeat_times, axis=0)
