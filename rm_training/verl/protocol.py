    ...
    def check_consistency(self):
        ...
        if self.batch is not None and len(self.non_tensor_batch) != 0:
            ...
            for key, val in self.non_tensor_batch.items():
                ...
                # Add debug logging before the failing assertion
                if val.shape[0] != batch_size:
                    print(f"DEBUG: Batch size mismatch detected!")
                    print(f"  key: {key}")
                    print(f"  val.shape[0]: {val.shape[0]}")
                    print(f"  expected batch_size: {batch_size}")
                    print(f"  tensor batch keys: {list(self.batch.keys()) if self.batch else []}")
                    print(f"  non_tensor batch keys: {list(self.non_tensor_batch.keys())}")
